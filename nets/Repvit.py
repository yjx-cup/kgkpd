    ...
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_hs):
        ...
        if stride == 2:
            self.token_mixer = nn.Sequential(
                ...
                # ELA(inp, "B") if use_se else nn.Identity(),
...
class repvit_FPN(nn.Module):
    def __init__(self, pretrained=True):
        super(repvit_FPN, self).__init__()
        model = repvit_m1_1(pretrained=pretrained)
        self.model = model
    ...
    def forward(self, x):
        ...
        for idx, block in enumerate(self.model[1:]):
            x = block(x)
            if block.token_mixer[0].__class__.__name__ == "Conv2d_BN":
                if block.token_mixer[0].c.stride == (2, 2):
...
class Conv2dStaticSamePadding(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True, groups=1, **kwargs):
        super().__init__()
