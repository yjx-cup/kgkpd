...
def GhostNetV3():
    ...
    features = list([model.conv_stem, model.bn1, model.blocks])
    features = nn.Sequential(*features)

    return features

class ghostNetV2_Decoder(nn.Module):
    def __init__(self, inplanes, bn_momentum=0.1):
        super(ghostNetV2_Decoder, self).__init__()
        self.bn_momentum = bn_momentum
        self.inplanes = inplanes
        self.deconv_with_bias = False

        # ----------------------------------------------------------#
        #   16,16,960 -> 32,32,256 -> 64,64,128 -> 128,128,64
...
class ghostNetV3_Head(nn.Module):
    def __init__(self, num_classes=80, channel=64, bn_momentum=0.1):
        super(ghostNetV3_Head, self).__init__()
        # -----------------------------------------------------------------#
