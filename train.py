...
from nets.model import CenterNet_HourglassNet, CenterNet_Resnet50, CenterNet_EfficentNetV2, \
    CenterNet_GhostNetV3, CenterNet_Repvit
from nets.model_training import get_lr_scheduler, set_optimizer_lr
...
if __name__ == "__main__":
    ...
    # mixed precision
    fp16            = False
    # category files storage path
    classes_path    = ""
    # model files storage path
    model_path      = ""
    input_shape     = [512, 512]
    # RepVit, ResNet50, EfficientNetV2, GhostNetV3, Hourglass101
    backbone        = "RepVit"
    pretrain        = False
    Init_Epoch          = 0
    Freeze_Epoch        = 150
    ...
    save_dir            = 'logs'
    num_workers         = 4
    # Use the path of the VOC-formatted dataset
    ...
    if backbone == "resnet50":
        model = CenterNet_Resnet50(num_classes, pretrained = pretrain)
    elif backbone == "EfficientNetV2":
        model = CenterNet_EfficentNetV2(pretrained=False, num_classes=num_classes)
    elif backbone == "GhostNetV3":
        model = CenterNet_GhostNetV3(num_classes=num_classes)
    elif backbone == "RepVit":
        model = CenterNet_Repvit(pretrained=pretrain, num_classes=num_classes)
    else:
        model = CenterNet_HourglassNet({'hm': num_classes, 'wh': 2, 'reg':2}, pretrained = pretrain)
    if model_path != '':
        if local_rank == 0:
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, optimizer, epoch,
