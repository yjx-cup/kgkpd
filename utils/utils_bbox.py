...
file_path = os.path.join(path_project, "Semantic Consistency/Stored matrices/my_dataset_kg_55_info.json")
with open(file_path, 'r') as j:
    info = json.load(j)
KG_VOC_info = info['KG_My_DataSet_info']
...
def decode_bbox(pred_hms, pred_whs, pred_offsets, confidence, cuda):
    ...
    for batch in range(b):
        ...
        if cuda:
            ...
            yv      = yv.cuda()
        # RPKG module
        heat_map = handle_heat_map(heat_map, pred_wh, pred_offset, xv, yv)
        ...
        detect = torch.cat([bboxes, torch.unsqueeze(class_conf[mask], -1), torch.unsqueeze(class_pred[mask], -1).float()],
                           dim=-1)
        detects.append(detect)

    return detects

def get_p_hat(predictions, boxes, bk=5, lk=5, e=0.75):
    ...
    box_centers = boxes.to(device)
    box_nearest = torch.empty(size=(num_boxes, bk), dtype=torch.long).to(device)
    for i in range(len(box_centers[:, 0])):
        box_center = box_centers[i]
        distances = torch.sqrt(
            (box_center[0] - box_centers[:, 0]) ** 2 + (box_center[1] - box_centers[:, 1]) ** 2)
    ...
    p_hat_init = torch.full(size=(num_boxes, num_classes), fill_value=(1 / num_classes),
                            dtype=torch.float64).to(device)
    p_hat = p_hat_init
    p_hat_temp = torch.clone(p_hat).to(device)
    ...
    for b in range(num_boxes):
        ...
        HHH = torch.squeeze(torch.div(num, denom)) + predictions[b]
        p_hat[b] = (1 - e) * torch.div(predictions[b], HHH) + e * predictions[b]
    return p_hat

def handle_heat_map(heatmap, prewh, preoffset, xv, yv):
    ############################################
    #
    # Here, we intervene using prior knowledge.
    #
    ############################################
    return heatmap
