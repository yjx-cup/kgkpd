...
import torch
from nets.model_training import focal_loss, reg_l1_loss, poly_loss
from tqdm import tqdm

from utils.utils import get_lr


def fit_one_epoch(model_train, model, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, backbone, save_period, save_dir, local_rank=0):
    total_r_loss    = 0
    total_c_loss    = 0
    total_loss      = 0
    val_loss        = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            if backbone=="resnet50":
                ...
                hm, wh, offset = model_train(batch_images)
                c_loss = focal_loss(hm, batch_hms)
                wh_loss = 0.1 * reg_l1_loss(wh, batch_whs, batch_reg_masks, smooth=True)
                off_loss = reg_l1_loss(offset, batch_regs, batch_reg_masks)
                loss = c_loss + wh_loss + off_loss
                total_loss += loss.item()
                total_c_loss += c_loss.item()
